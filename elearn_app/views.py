...
from elearn_app.models import Course
from elearn_app.forms import CourseForm
    ...
    if request.method == "POST":
        ...
        if user_form.is_valid() and profile_form.is_valid():
            try:
                ...
                return render(request, "elearn/signup.html", {"user_form": user_form, "profile_form": profile_form, "registered": registered, "error": "You already have an account. Please login instead."})

        else:
            return render(request, "elearn/signup.html", {"user_form": user_form, "profile_form": profile_form, "registered": registered, "error": "Please fill in all entries in the form to register."})
            print(user_form.errors, profile_form.errors)
    else:
        user_form = UserForm()
        profile_form = UserProfileForm()
    # load template signup.html
    return render(request, "elearn/signup.html", {"user_form": user_form, "profile_form": profile_form, "registered": registered})
...
class UserList(ListView):
    model = User
    template_name = "elearn/users.html"
...
class CourseCreate(PermissionRequiredMixin, CreateView):
    ...
    success_url = reverse_lazy('course_list')  # Replace 'course_list' with the actual name of your course list URL

    def dispatch(self, request, *args, **kwargs):
        # Check if the user has the required permission
        if not request.user.has_perm(self.permission_required):
            return HttpResponse(status=404)  # Or return a 403 Forbidden error if you prefer
        return super().dispatch(request, *args, **kwargs)
...
class CourseList(ListView):
    ...
    model = Course
    template_name = "elearn/courses.html"
...
def enroll(request, pk):
    ...
    return HttpResponseRedirect(reverse('courses.show', kwargs={'pk': pk}))
...
class FeedbackCreate(CreateView):
    ...
    def get_success_url(self):
        return reverse('courses.show', kwargs={'pk': self.kwargs['courseid']})
